...
def train(opt):
    ...
    training_set = COCODataset(opt.data_path, opt.image_size)
    training_generator = DataLoader(training_set, **training_params)

    test_set = COCODataset(opt.data_path, opt.image_size, is_training=False)
    ...
    for epoch in range(opt.num_epoches):
        ...
        if epoch % opt.test_interval == 0:
            ...
            if te_loss + opt.es_min_delta < best_loss:
                best_loss = te_loss
                best_epoch = epoch
                torch.save(model.state_dict(), opt.saved_path + os.sep + "only_params_trained_deeplab_coco")
                torch.save(model, opt.saved_path + os.sep + "whole_model_trained_deeplab_coco")
